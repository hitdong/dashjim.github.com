...
$J(w,b)=\frac{1}{m}\sum\limits_{i=1}^{m}{L\left({\hat{y}^{(i)}}, {y}^{(i)} \right )}=\frac{1}{m}\sum\limits_{i=1}^{m}{\left ( -{{y}^{(i)}}\log {\hat{y}^{(i)}}-\left(1-{{y}^{(i)}} \right )\log\left(1-\hat{y}^{(i)} \right ) \right )}$
